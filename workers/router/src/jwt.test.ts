...
describe('JWT utilities', () => {
	...
	describe('generateJWT', () => {
		...
		it('includes payload data in token', async () => {
			...
			const verified = await verifyJWT<JwtPayload>(token, secret);
	...
	describe('verifyJWT', () => {
		...
		it('returns undefined for invalid signature', async () => {
		...
		it('returns undefined for malformed token', async () => {
			expect(await verifyJWT('not.a.valid.jwt.token', secret)).toBeUndefined();
			expect(await verifyJWT('malformed', secret)).toBeUndefined();
			expect(await verifyJWT('', secret)).toBeUndefined();
		});

		it('returns undefined for token with missing parts', async () => {
			expect(await verifyJWT('header.payload', secret)).toBeUndefined();
			expect(await verifyJWT('header.', secret)).toBeUndefined();
			expect(await verifyJWT('.payload.signature', secret)).toBeUndefined();
		});
		...
		it('returns null for tampered token', async () => {
			...
			const parts = token.split('.');
			parts[1] = parts[1]?.slice(0, Math.max(0, parts[1].length - 2)) + 'XX';
			const tamperedToken = parts.join('.');

			const verified = await verifyJWT<Record<string, unknown>>(tamperedToken, secret);
			expect(verified).toBeUndefined();
		});
		...
		it('different secrets produce different tokens', async () => {
			...
			expect(await verifyJWT(token1, 'secret2')).toBeUndefined();
			expect(await verifyJWT(token2, 'secret2')).toBeDefined();
			expect(await verifyJWT(token2, 'secret1')).toBeUndefined();
		});
	});
