...
describe('search methods', async () => {
	describe('binary search', () => {
		it("doesn't error for an empty manifest", async () => {
			...
			const foundEntry = binarySearch(new Uint8Array(manifest), await hashPath('/path'));
		...
		it('throws an error if the search value is longer than the current value', async () => {
			const { manifest } = await makeManifestOfLength(10);
			expect(() => {
				binarySearch(new Uint8Array(manifest), new Uint8Array(PATH_HASH_SIZE + 1).fill(127));
			}).toThrowErrorMatchingInlineSnapshot(`[TypeError: Search value should have a length of ${PATH_HASH_SIZE}]`);
		});
	});
});
